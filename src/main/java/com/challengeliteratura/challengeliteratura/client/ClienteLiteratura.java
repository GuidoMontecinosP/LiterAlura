...
public class ClienteLiteratura {
    ...
    private void buscarLibroWeb() {
        Respuesta datos = getDatosSerie(); // Obtiene los datos de la API

        if (!datos.results().isEmpty()) {
            System.out.println("Libros encontrados:");
            int index = 1;

            for (var libro : datos.results()) {
                String titulo = libro.title();
                String idioma = String.join(", ", libro.languages());
                String autor = libro.autores().isEmpty() ? "Desconocido" : libro.autores().get(0).name();
                System.out.printf("%d. %s (Idioma: %s, Autor: %s)%n", index++, titulo, idioma, autor);

                // Verificar si el libro ya existe en la base de datos
                if (!libroRepositorio.existsByTituloAndLenguaje(titulo, idioma)) {
                    // Crear y guardar el libro si no existe
                    LibroEntity libroEntity = new LibroEntity(libro);
                    libroRepositorio.save(libroEntity);
                }
            }
        } else {
            System.out.println("\n\n ----- NO SE ENCONTRARON RESULTADOS ---- \n\n");
